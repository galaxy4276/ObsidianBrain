...
# 레이어드 아키텍처의 문제
흔하게 Controller -Service - Repository 구조로 사용하던 문제는 서로의 의존성이 강하고 DB 중점 개발, 도메인이 죽어버린다는 문제가 있다. (OOP 도 해친다.)

![[Pasted image 20231202194912.png]]

위 구조를 다음과 같이 생각하는 습관을 들여야 한다.

![[Pasted image 20231202194934.png]]

인터페이스에 의존하기 때문에 Mock 과정 처리가 자연스러워지며,
도메인 계층은 순수한 자바 코드로 서비스에 강하게 의존적인 코드를 피할 수 있다.
